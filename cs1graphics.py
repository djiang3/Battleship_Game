...
Copyright 2011, David Letscher and Michael H. Goldwasser

Go to www.cs1graphics.org for more information.

This is Version 1.2a1+ alpha bugfix release (17 October 2011)
"""
...
class _GraphicsManager:
    ...
    def processCommands(self):
        try:
            while self._running and not self._commandQueue.empty():
                command = self._commandQueue.get(False)
                try:
                    self.processCommand(command)
                except GraphicsError:
                    raise
        except _Queue.Empty:   # TODO:  why does this happen, and why set running to False?
            if _debug >= 1:
    ...
    def mainLoop(self, waiting=None, exitOnAllClosed=True):
        ...
        while self._running and self._handlingEvents in ('Yes', 'Waiting'):
            _tkroot.update()
            self.processEvents()
...
class _RenderedCanvas(object):
    ...
    def _handleEvent(self, event):
        ...
        if len(tkIds) > 0:
            chain = _graphicsManager._objectIdRegistry[tkIds[-1]]._chain
        else:          
...
class _RenderedCircle(_RenderedFillableShape):
    def __init__(self, chain, properties):
        ...
        statement += 'smooth=1)'
        exec(statement)
        _graphicsManager._objectIdRegistry[self._object] = self
